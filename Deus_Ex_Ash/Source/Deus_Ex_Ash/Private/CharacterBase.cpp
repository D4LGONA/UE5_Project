...
void ACharacterBase::ApplyDamage(AActor* AttackerCharacter, AActor* Projectile, FVector ImpactPoint, float Damage, float StaggerDuration, bool IgnoreGuard)
{
	...
	if (EquippedWeaponBase && AttackerCharacterBase && AttackerWeaponBase)
	{
		...
		else if (Dot < FMath::Cos(PI / 4) && Dot > FMath::Cos(PI * 3 / 4))
		{
			if (CrossZ < 0)
			{
				HitDirection = ECharacterDirection::Left;
			}
			else if (CrossZ > 0)
			{
				HitDirection = ECharacterDirection::Right;
			}
		}
		else
		{
		...
		{
			Hit(AttackerCharacterBase, HitDirection, Damage, 0.0f, false);
		}
		else
		{
