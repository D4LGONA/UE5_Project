...
void AWeaponBase::Fire(FProjectileData ProjectileData, AActor* TargetActor, bool bIsHoming)
{
	if (ProjectileData.Class)
	{
		...
		if (World)
		{
			...
			AProjectileBase* Projectile = World->SpawnActor<AProjectileBase>(ProjectileData.Class, ProjectileData.SpawnPoint->GetComponentLocation(), ProjectileData.SpawnPoint->GetComponentRotation(), SpawnParams);
			if (Projectile)
			{
				if (TargetActor)
				{
					if (bIsHoming)
					{
						Projectile->ProjectileMovementComponent->HomingTargetComponent = TargetActor->GetRootComponent();
					...
					FVector Direction = (TargetActor->GetActorLocation() - ProjectileData.SpawnPoint->GetComponentLocation()).GetSafeNormal();
					Projectile->SetFireDirection(Direction);
				}
				else
				{
					Projectile->ProjectileMovementComponent->bIsHomingProjectile = false;
					Projectile->SetFireDirection(ProjectileData.SpawnPoint->GetComponentRotation().Vector());
				}

				Projectile->ProjectileDamage = ProjectileData.Damage;
				Projectile->CollisionComponent->SetCollisionEnabled(ECollisionEnabled::QueryOnly);
			}
			else
			{
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Orange, TEXT("Projectile is Null"));
			}
		}
	}
}